...
md"""$\texttt{Figura 1. Casa Pashkov. Imagen tomada de Wikipedia.}$"""
...
function metodo_gradiente(A)
    ...
    for m in 1:M-1
        for n in 1:N-1
            Gx[m, n] = A[m, n + 1] - A[m, n]   # Gradiente horizontal
            Gy[m, n] = A[m + 1, n] - A[m, n]   # Gradiente vertical
            G[m, n] = sqrt(Gx[m, n]^2 + Gy[m, n]^2) # Magnitud del gradiente
        end
    end

    return abs.(G), abs.(Gx), abs.(Gy)
end
...
begin
    [Gray.(A) RGB.(ones(size(A)[1], 10)) Gray.(Gx);
     RGB.(ones(10, size(A)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(A)[2]));
     Gray.(Gy) RGB.(ones(size(A)[1], 10)) Gray.(1 .- G)]
end

# ╔═╡ fcab4790-2f0c-4f70-bff6-ce0c8b878762
md"""$\texttt{Figura 2. Aplicación del método del gradiente}$"""

# ╔═╡ 8670370b-b8a8-405d-930f-eb52d7bc878a
md"""
En la esquina superior izquierda encontramos la imagen original, a su lado observamos la aplicacion del gradiente horizontal, en la esquina inferior izquierda vemos la aplicacion del grandiente vertical, y finalmente, a su costado, la magnitud del gradiente.
"""
...
Finalmente, cuando se calcula la $\underline{\text{magnitud}}$ del gradiente, se combinan los cambios de intensidad en ambas direcciones, y es precisamente allí cuando aparecen los bordes de la foto con claridad.
"""
...
begin
    [Gray.(B) RGB.(ones(size(B)[1], 10)) Gray.(Gx1);
     RGB.(ones(10, size(B)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(B)[2]));
     Gray.(Gy1) RGB.(ones(size(B)[1], 10)) Gray.(1 .- G1)]
end

# ╔═╡ 17763dc7-d98c-4bf4-88ec-d155d61c67dc
md"""$\texttt{Figura 3. Aplicación del método del gradiente}$"""
...
begin
    [Gray.(C) RGB.(ones(size(C)[1], 10)) Gray.(Gx2);
     RGB.(ones(10, size(C)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(C)[2]));
     Gray.(Gy2) RGB.(ones(size(C)[1], 10)) Gray.(1 .- G2)]
end

# ╔═╡ fe9888f8-c02c-4c5d-870c-8d211f1507f1
md"""$\texttt{Figura 4. Imagen tomada de [2]. Aplicación del método del gradiente.}$"""
...
md"""$\texttt{Figura 5. Piramides de Guiza. Imagen tomada de [3].}$"""
...
function roberts(A)
	...
	for m in 1:M-1
		for n in 1:N-1
			G1[m, n] = A[m+1, n+1] - A[m, n]; #Diagonal en sentido positivo
			G2[m, n] = A[m+1, n] - A[m, n+1]; #Diagonal en sentido negativo
...
begin
    [Gray.(D) RGB.(ones(size(D)[1], 10)) Gray.(G1_r);
     RGB.(ones(10, size(D)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(D)[2]));
     Gray.(G2_r) RGB.(ones(size(D)[1], 10)) Gray.(G_r)]
end

# ╔═╡ 4dfe3396-1019-43e4-9758-6a76ed476cfb
begin
	url4="https://github.com/ytrujillol/Procesamiento-de-imagenes/blob/main/Images/guiza.png?raw=true"
	fname4 = download(url4)
	E = Gray.(load(fname4));
end

# ╔═╡ d70efa1d-1404-42dc-b02f-3e5a42935227
G_r2,G1_r2,G2_r2 = roberts(E);

# ╔═╡ f8e7e5bb-d8d6-406f-9e38-26c950d525fb
begin
    [Gray.(E) RGB.(ones(size(E)[1], 10)) Gray.(G1_r2);
     RGB.(ones(10, size(E)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(E)[2]));
     Gray.(G2_r2) RGB.(ones(size(E)[1], 10)) Gray.( G_r2)]
end

# ╔═╡ 12f4cd0b-9e90-4a42-8f6b-783ef61686ae
G_r3,G1_r3,G2_r3 = roberts(A);

# ╔═╡ 11e42302-6fa5-4758-83b4-5e454df1410b
begin
    [Gray.(A) RGB.(ones(size(A)[1], 10)) Gray.(G1_r3);
     RGB.(ones(10, size(A)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(A)[2]));
     Gray.(G2_r3) RGB.(ones(size(A)[1], 10)) Gray.(1 .- G_r3)]
end

# ╔═╡ 7c0ae831-b147-4210-9b71-9d2bad84a740
[Gray.(A) Gray.(A); Gray.(1 .- G) Gray.(1 .- G_r3)]

# ╔═╡ cae0a996-e619-4724-8603-d9ff3d1a3afc
G_r4,G1_r4,G2_r4 = roberts(C);

# ╔═╡ 0ac2047c-6641-4bad-a388-c37eb142bbe9
[Gray.(C) Gray.(C); Gray.(1 .- G2) Gray.(1 .- G_r4)]

# ╔═╡ e5ffda18-1d52-4aae-8c35-88bd62ca7a5b
G_r5,G1_r5,G2_r5 = roberts(B);

# ╔═╡ f79326ba-bede-4d96-95e8-c9f3d007e70e
G6, Gx6, Gy6 = metodo_gradiente(D);

# ╔═╡ dd78812e-eae1-4c62-a617-2f41e2e7c994
[Gray.(D) Gray.(G6)]

# ╔═╡ 46ea2d31-6fa7-4b9d-877d-c3da54872433
begin
	url5="https://github.com/ytrujillol/Procesamiento-de-imagenes/blob/main/Images/mountain.jpg?raw=true"
	fname5 = download(url5)
	F = Gray.(load(fname5));
end

# ╔═╡ 107cf5d8-00bb-43b6-8d22-ebb95efb2138
G7, Gx7, Gy7 = metodo_gradiente(F);

# ╔═╡ 8847e216-7757-4950-9bf4-f90cb82ecb5d
G_r7,G1_r7,G2_r7 = roberts(F);

# ╔═╡ 4c227f0e-9686-4ef3-9f62-fd57e798f10f
[Gray.(F) Gray.(F); Gray.(1 .- G7) Gray.(1 .- G_r7)]
...
# ╔═╡ 410606eb-e811-49ed-8396-e51e62ad4a5a
[Gray.(F) Gray.(F) ; Gray.(1 .- G7) metodo_prewitt(F)]
...
[Gray.(A) Gray.(A) ; metodo_prewitt(A) metodo_sobel(A)]

# ╔═╡ 5894d279-22d4-4aab-8b7a-11e2137e0341
[Gray.(F) Gray.(F) Gray.(F); Gray.(1 .- G7) metodo_prewitt(F) metodo_sobel(F)]
...
md"""ocupando asi una sola convolución a diferencia de los métodos expuestos anteriormente."""
...
[B metodo_sobel(B) laplaciano(B)]

# ╔═╡ 43629324-34fc-413d-b838-6ad8f95eaca1
[C metodo_sobel(C) laplaciano(C)]

# ╔═╡ 206a01c2-6621-4075-b075-84e44d2f6c07
[F metodo_sobel(F) laplaciano(F)]

# ╔═╡ f68ace2b-e1da-4f90-8a2d-9affbad14e94
[D metodo_sobel(D) laplaciano(D)]
...
begin
	Gn, Gxn, Gyn = metodo_gradiente(ruido_gaussiano(A, 0.15));
	Gn1, Gxn1, Gyn1 = metodo_gradiente(ruido_gaussiano(B, 0.15));
	Gn2, Gxn2, Gyn2 = metodo_gradiente(ruido_gaussiano(C, 0.15));
	Gn3, Gxn3, Gyn3 = metodo_gradiente(ruido_gaussiano(F, 0.15));
	nothing
end

# ╔═╡ b5148658-c15d-4d80-9f53-a55135a568a1
[Gray.(1 .- Gn) metodo_prewitt(ruido_gaussiano(A, 0.15)); metodo_sobel(ruido_gaussiano(A, 0.15)) laplaciano(ruido_gaussiano(A, 0.15))]

# ╔═╡ e2bee96d-3123-4710-aae3-fc93fd7e2340
[Gray.(1 .- Gn1) metodo_prewitt(ruido_gaussiano(B, 0.15)); metodo_sobel(ruido_gaussiano(B, 0.15)) laplaciano(ruido_gaussiano(B, 0.15))]

# ╔═╡ af1a14fe-cf9c-4a7c-ae2d-10ed4aa852fa
[Gray.(1 .- Gn2) metodo_prewitt(ruido_gaussiano(C, 0.15)); metodo_sobel(ruido_gaussiano(C, 0.15)) laplaciano(ruido_gaussiano(C, 0.15))]

# ╔═╡ 425cf899-7563-4bec-a868-17acb581d258
[Gray.(1 .- Gn3) metodo_prewitt(ruido_gaussiano(F, 0.15)); metodo_sobel(ruido_gaussiano(F, 0.15)) laplaciano(ruido_gaussiano(F, 0.15))]

# ╔═╡ 7f524bae-a288-4488-9f48-2e675e973892
md"""Una forma razonable de abordar este problema es intentar reducir el nivel de ruido antes de calcular los bordes, utilizando los métodos descritos en el cuaderno $\texttt{Difuminado y Ruido}$. Apliquemos el **Filtro Gaussiano**:"""
...
md"""
Antes de pasar a la práctica y crear una función de filtrado, resulta útil analizar el problema de manera analítica. Esta mejora parece implicar un costo en términos de mayor complejidad computacional, ya que el proceso:
"""
...
$
\begin{align*}LdG &= L*W \\ \\ 
&= \large \begin{bmatrix} -1 & -1 & -1 \\ -1 & 8 & -1 \\ -1 & -1 & -1 
\end{bmatrix} * \large \begin{bmatrix} \frac{2}{25} & \frac{3}{25} & \frac{2}{25} \\ \frac{3}{25} & \frac{5}{25} & \frac{3}{25} \\ \frac{2}{25} & \frac{3}{25} & \frac{2}{25} 
...
[Gray.(1 .- Gn) LaplaGauss(1 .- Gn)]

# ╔═╡ ee7cfd7c-2ef7-4425-a6c7-42eeb8f356ae
[Gray.(1 .- Gn2) LaplaGauss( Gn2)]

# ╔═╡ 955471d4-a24a-40f8-bd61-96fdd8abe868
[Gray.(1 .- Gn3) LaplaGauss(Gn3)]
...
[Gray.(C) LaplaGauss(1 .- C)]

# ╔═╡ 9f8426b3-7d6c-49f6-b614-c565f5060204
[Gray.(D) LaplaGauss(1 .- D)]

# ╔═╡ 34365a62-4812-47f3-9e33-ac9536847886
[Gray.(F) LaplaGauss(1 .- F)]
...
manifest_format = "2.0"
project_hash = "51c834a5012bbf99bde0cd64c2cd6bfe64952085"
