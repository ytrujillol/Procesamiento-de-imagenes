...
$(A * B)_{i,j} = \sum_{u} \sum_{v} A_{u,v} \cdot B_{i-u, j-v}$
"""
...
function convolucion_matrices(A,B)
    m1, n1 = size(A)
    m2, n2 = size(B)
    resultado = zeros((m1 + m2 - 1, n1 + n2 - 1))
    
    for i in 1:(m1 + m2 - 1)
        for j in 1:(n1 + n2 - 1)
			for u in 1:m1
	            for v in 1:n1
					if 1<=i+1-u<=m2 && 1<=j+1-v<=n2
						resultado[i, j] += A[u,v] * B[i+1-u,j+1-v]
					end
				end
	        end 
        end
...
# ╔═╡ 96ebf5a6-a68a-4e3e-a1c9-0c99ad2fdabf
md"""
## Convolución como una transformación lineal
"""

# ╔═╡ 1796c76e-9b0b-4f9b-be1d-fb35d1f745c2
md"""
...
# ╔═╡ b59a167c-375a-409e-a030-7c43c7d893cf
begin
...
begin
	Image1 = Gray.(imresize(testimage(TestImages.remotefiles[15]), (60, 100)))
	plot(Image1, axis=false, grid=false, size=(600,400))
end
...
# ╔═╡ f7ae52c9-4131-4da1-bdb1-0e40cf5b7a7d
plot(Gray.(convolucion_matrices(A,kernelb)), axis=false, grid=false, size=(600,400))

# ╔═╡ 75100b66-840a-4586-b776-36fb60bfd23d
md"""$\texttt{Figura 5. Resultado de aplicar el filtro a la Figura 4 con nuestra función.}$"""
...
md"""Material tomado de [6]. En esta sección probamos diferentes kernels a varias imágenes. Cada kernel tiene una interpretación diferente y se construirá teóricamente en otro cuaderno asociado a este seminario."""
...
# ╔═╡ 8de8041e-2a0b-4d2a-b9cb-705a516e997f
md"""
En la siguiente imagen se muestra a la izquierda la imagen original del billete de 100 dólares americanos y a la derecha el resultado de aplicar el kernel previamente seleccionado.
"""
...
md"""$\texttt{Figura 8. Aplicación del kernel seleccionado}$"""

# ╔═╡ a1d0ef13-86cf-4286-ae30-c86ee51894d6
md"""
En la siguiente figura se muestra:
- En la esquina superior izquierda la imagen original de un gato.
- En la esquina superior derecha la imagen convolucionada con un kernel gaussiano de con $\sigma =2$ como desviación estandar.
- En la esquina inferior izquierda la imagen convolucionada con un kernel gaussiano de con $\sigma =15$ como desviación estandar.
- En la esquina inferior derecha se encuentra el negativo de la imagen convolucionado con un kernel laplaciano.
"""
...
begin
	...
	imfilter(gato,Kernel.gaussian(15)) RGB.(ones(size(gato)[1], 10)) Gray.(1 .- channelview(Gray.(imfilter(gato,Kernel.Laplacian()))))]
end

# ╔═╡ 0571b4d4-ec84-4d17-ba16-d8126f63fa9b
md"""$\texttt{Figura 9.}$"""

# ╔═╡ fa03a114-81c3-4cc2-9c11-78ea01ab4f3c
md"""
Al igual que en la figura anterior, realizamos el mismo procedimiento para la imagen de un astronauta.
"""

# ╔═╡ 1f905eb1-00fe-4d40-b682-10c8183b1031
begin
	urlastro ="https://github.com/ytrujillol/Procesamiento-de-imagenes/blob/main/Images/astronaut.jpg?raw=true"
	fnameastro = download(urlastro)
	astro = Gray.(load(fnameastro))
	
	[astro RGB.(ones(size(astro)[1], 10)) imfilter(astro,Kernel.gaussian(2));
	RGB.(ones(10, size(astro)[2])) RGB.(ones(10, 10)) RGB.(ones(10, size(astro)[2]));
	imfilter(astro,Kernel.gaussian(15)) RGB.(ones(size(astro)[1], 10)) Gray.(1 .- channelview(Gray.(imfilter(astro,Kernel.Laplacian()))))]
end

# ╔═╡ 5b9993e8-bfce-4bcb-8f01-332108805603
md"""$\texttt{Figura 10.}$"""

# ╔═╡ 060a7305-ef1e-4c7b-8f2d-3c1cc20bbaf1
md"""
Cabe destacar que la función $\texttt{imfilter}$ toma una imagen y un kernel, y retorna una imagen del mismo tamaño que la imagen original. Sin embargo, sabemos que la operación de convolución generalmente produce una matriz más grande debido a la aplicación del filtro en los bordes de la imagen. Para evitar este problema, la función de la librería expande la imagen de alguna manera, comúnmente mediante un proceso de relleno (padding), en el cual se agregan ceros (o cualquier otro valor) alrededor de la imagen original. Luego, se realiza la convolución sobre esta imagen expandida, y finalmente se recorta o ajusta la salida para que tenga el mismo tamaño que la imagen original, garantizando así que no se pierdan información en los bordes sin aumentar el tamaño de la imagen resultante.
En la siguiente figura de izquierda a derecha y de arriba a abajo se muestra: un relleno que replica los bordes, un relleno circular que copia la imagen, un relleno simétrico que refleja la imagen y un relleno nulo.
"""

# ╔═╡ 1ab0b0f7-6d94-4e18-be27-bf553de078e7
begin
	zer = Gray.(kron([0 0 0; 0 1 0; 0 0 0], channelview(astro)))[3166:end-3167,2001:end-2000]
	
	[padarray(astro, Pad(:replicate,1000,1000)) Gray.(ones(6166, 100)) padarray(astro, Pad(:circular,1000,1000)); 
	Gray.(ones(100, 5000)) Gray.(ones(100, 100)) Gray.(ones(100, 5000));
	padarray(astro, Pad(:symmetric,1000,1000)) Gray.(ones(6166, 100)) zer;]
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "4e1a4ed613d9287f9d7ed205344c5701ff01a3d2"
