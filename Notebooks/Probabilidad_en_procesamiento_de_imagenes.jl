...
# ╔═╡ 00e10a32-966b-11ef-1063-637f87ef031e
using PlutoUI

# ╔═╡ 946c9e90-f8f5-4a00-86e3-56a7071bad12
begin
...
# ╔═╡ bb88216a-0777-4f03-a62e-27c605b827d6
PlutoUI.TableOfContents(title="Probabilidad, variables aleatorias, y procesamiento de histogramas", aside=true)

# ╔═╡ cd02f9f9-348d-44c2-a45f-a18d1d3d0686
md"""Este cuaderno está en construcción y puede ser modificado en el futuro para mejorar su contenido. En caso de comentarios o sugerencias, por favor escribir a **labmatecc_bog@unal.edu.co**.

Tu participación es fundamental para hacer de este curso una experiencia aún mejor."""

# ╔═╡ 88cdc10d-34aa-44aa-90fa-d46640e9f5c9
...
# ╔═╡ 17a0a4b8-0544-496f-a775-24fe2a749e56
md"""Vamos a usar las siguientes librerías:"""

# ╔═╡ 3dab18c1-722c-40e5-bd9e-110a167f0381
md"""Funciones auxiliares:"""

# ╔═╡ 23f4645d-27dc-47cf-a47c-911dae00214f
begin
...
# ╔═╡ 847554d7-66cc-4b32-8aa2-4a39f66a6152
begin
...
# ╔═╡ f2f30d34-d729-4e35-9a64-fb58b222a1ea
begin
...
# ╔═╡ 6700cd59-fa3e-4c88-9ab9-813665b04d39
md"""
# Motivación
"""

# ╔═╡ 33289807-3605-499d-a38e-a494e2eedd2e
md"""Anteriormente, exploramos cómo oscurecer imágenes sobreexpuestas, aclarar las subexpuestas y mejorar el contraste utilizando funciones elementales que se suelen estudiar en cursos de precálculo, como las funciones lineales, las funciones a trozos lineales, así como las funciones de potencia y logarítmicas. Estas transformaciones ayudan a que los histogramas sesgados se vuelvan más simétricos, mientras que los histogramas con una alta concentración cerca de un valor central tienden a volverse más uniformes.

En este cuaderno, aprenderemos a mejorar las imágenes especificando el tipo de histograma que deseamos para nuestra imagen. Posteriormente, diseñaremos una transformación que produzca una versión de la imagen con el histograma deseado, utilizando métodos de probabilidad."""

# ╔═╡ 0c9081fa-9097-4872-9558-f068440acea4
md"""Para cualquier imagen digital $A$, podemos definir una variable aleatoria asociada de la siguiente manera: seleccionemos un píxel al azar. La variable aleatoria $X$ se definirá como el valor de ese píxel seleccionado aleatoriamente. Los valores posibles de $X$ son los enteros de 0 a 255 (correspondientes a los valores de escala de grises posibles), y las probabilidades de esos valores son proporcionadas por el histograma de la imagen."""

# ╔═╡ ed45ee1e-061d-4890-82da-0016b8af93ac
md"""**Definición:** Supongamos que $X$ es una variable aleatoria continua con la función de distribución acumulada $F(x)$. Entonces, su función de densidad de probabilidad (PDF) se denota por $f(x)$ y se define por

$f(x) = F'(x).$
La gráfica de la función de densidad de probabilidad de una variable aleatoria $X$ se llama "curva de densidad" de $X$.

De la anterior definición y del Teorema Fundamental del Cálculo se tiene que la función de densidad de probabilidad $f$ de la variable aleatoria continua $X$ se puede usar para calcular su función de distribución acumulada mediante

$F(x) = \int_{t=-\infty}^x f(t) \, dt.$
Note que se puede utilizar $f$ o $F$ para calcular la probabilidad de que $X$ caiga en el intervalo $[a, b]$ mediante

$P(a \leq X \leq b) = F(b) - F(a) = \int_a^b f(t) \, dt.$
Es igual de fácil visualizar las curvas de densidad de variables aleatorias continuas que visualizar las funciones de probabilidad de sus contrapartes discretas. Por ejemplo, si denotamos los valores de los píxeles seleccionados al azar por $X$, entonces el histograma de la imagen proporciona una representación gráfica de la función de probabilidad $p(x)$, mientras que el contorno suave del histograma de la imagen es precisamente la curva de densidad de $X$ (si decidimos tratarlo como una variable aleatoria continua). Esto se muestra a continuación.
"""
...
# ╔═╡ 826d9d3c-fe13-4815-a57f-ceb25802b8c4
md"""$\texttt{Figura 1. Niño. Imagen tomada de [1].}$"""
...
# ╔═╡ 04d0fb7d-0610-4a3f-af6b-970f6da66f2d
md"""$\texttt{Figura 2. Curva de densidad de la Figura 1.}$"""
...
md"""
**Teorema de transformación de variables aleatorias continuas.** 

Suponga que $X$ es una variable aleatoria continua con valores en el intervalo $[a,b]$. Se define la variable aleatoria $Y=g(X)$ donde $g$ es una función diferenciable y estrictamente creciente. Entonces

$f_{Y}(y) = \begin{cases}
f_{X}(g^{-1}(y)) \cdot \frac{1}{g'(g^{-1}(y))}, & g(a)\leq y \leq g(b),\\
0, & \text{en otro caso.}
\end{cases}$
"""
...
md"""El procedimiento de ecualización al aplicar la transformación a todos los valores de los píxeles ciertamente mejora significativamente la calidad visual de la imagen, aunque podríamos desear aumentar aún más el contraste de la imagen. Para ello, quisiéramos que el histograma de la imagen adopte una forma de "V" o "U", de modo que la mayoría de los píxeles se vuelvan muy oscuros o muy brillantes, con muy pocos permaneciendo en el rango medio."""

# ╔═╡ 9dadcf91-09e0-4568-9db2-7534fd0e6efe
function HistMatching(image, form)
	A = Int.(channelview(image)*255)
	B = fill(0.0,size(A))
	mpfX = [count(x -> x == t, vec(A))/prod(size(A)) for t in 0:255]
	if form=="E"
		mpfY = [1/256 for t in 0:255]
	end
	if form=="V"
		mpfY = [(abs(t - 127)+1)/16640 for t in 0:255]
	end
	if form=="U"
		mpfY = [(t - 127.5)^2 for t in 0:255]/sum([(t - 127.5)^2 for t in 0:255])
	end
	
	gp = [mpfX[t+1]/mpfY[t+1] for t in 0:255]
	g = [sum(gp[1:x+1]) for x in 0:255]
...
# ╔═╡ d043e777-dabb-416c-b7d6-26743fef2c1a
md"""
En la siguiente figura se muestra de izquierda a derecha la imagen de la niña original, la imagen equalizada, la imagen con histograma en forma de "V" y finalmente la imagen con histograma en forma de "U".
"""

# ╔═╡ 0ba59fb3-59ad-4e90-a4df-a0ac00cfc0a0
[Gray.(load(fname1)) HistMatching(Gray.(load(fname1)), "E") HistMatching(Gray.(load(fname1)), "V") HistMatching(Gray.(load(fname1)), "U")]

# ╔═╡ 63b25ba7-3229-4fc0-8c8a-53fa5b2676f0
md"""
En la siguiente figura se muestra de en la esquina superior izquierda el histograma de la imagen de la niña original, en la esquina superior derecha el histograma de la imagen equalizada, en la esquina inferior izquierda el histograma de la imagen con histograma en forma de "V" y finalmente, en la esquina inferior derecha el histograma de la imagen con histograma en forma de "U".
"""

# ╔═╡ 8d80a910-833b-468d-b35d-4836e19d8b20
begin
	p11 = Hist(Gray.(load(fname1)))
	p12 = Hist(HistMatching(Gray.(load(fname1)), "E")) 
	p21 = Hist(HistMatching(Gray.(load(fname1)), "V"))
...
md"""
Un ejemplo con otra imagen sobrexpuesta:
"""

# ╔═╡ 2de78726-1230-4a10-b302-5a2ca568b9d8
begin
	url3 = "https://github.com/ytrujillol/Procesamiento-de-imagenes/blob/main/Images/Sobreexpuesta2.jpg?raw=true"
	fname3 = download(url3)
	Image3 = Gray.(load(fname3))
end

# ╔═╡ 97a852ae-ef08-40b9-a3f2-4dcdbaadb628
md"""
El resultado de la coincidencia de histogramas se muestra en la dos siguientes figuras.
"""

# ╔═╡ 4d3a7018-e0e2-4246-9282-061b2e79912e
[Gray.(load(fname3)) HistMatching(Gray.(load(fname3)), "E") HistMatching(Gray.(load(fname3)), "V") HistMatching(Gray.(load(fname3)), "U")]

# ╔═╡ 75cbffa9-0bdc-4e50-b2d9-42cbcae186c2
begin
	p113 = Hist(Gray.(load(fname3)))
	p123 = Hist(HistMatching(Gray.(load(fname3)), "E")) 
	p213 = Hist(HistMatching(Gray.(load(fname3)), "V"))
	p223 = Hist(HistMatching(Gray.(load(fname3)), "U"))
	plot(p113,p123,p213,p223, layout=(2,2))
end
...
manifest_format = "2.0"
project_hash = "a25fee7afa5a066ddbd20358d6bf4229ca234f32"
