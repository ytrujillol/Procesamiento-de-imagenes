...
# ╔═╡ 05f1d1ce-aa30-4f58-afe3-1cfd9e5bb7b7
md"""
...
md"""
Observemos la siguiente figura (figura 23). Parece que luce demasiado oscura. El problema es que luce demasiado "grisasea" y carece de contraste.
"""
...
md"""$\texttt{Figura 23. Alce en la niebla.}$"""
...
md"""$\texttt{Figura 24. Comparación de imágenes originales y transformadas}$"""
...
md"""$\texttt{Figura 25. Histogramas de imágenes originales y transformadas}$"""
...
md"""$\texttt{Figura 26. Comparación de imágenes originales y transformadas}$"""
...
md"""$\texttt{Figura 27. Histogramas de imágenes originales y transformadas}$"""
...
begin
	URL6 = "https://github.com/ytrujillol/Procesamiento-de-imagenes/blob/main/Images/alone-tree.jpg?raw=true"
...
md"""$\texttt{Figura 28. Arbol solitario.}$"""
...
md"""$\texttt{Figura 30. Comparación entre imagen original y ambas transformaciones}$"""
...
md"""
El efecto de *estiramiento de contraste* está directamente relacionado con la expresión $m(\gamma) = f'(x_0;\gamma) =\gamma x_{0}^{\gamma-1}$.
...
plot(0:0.001:1,[(cos(4*π*t)+1)/2 for t in 0:0.001:1],title="Función g",label="")
...
md"""$\texttt{Figura ??. Transformación sin sentido}$"""
...
md"""$\texttt{Figura ??. Histograma de la transformación sin sentido}$"""
